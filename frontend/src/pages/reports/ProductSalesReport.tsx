...
const ProductSalesReport = () => {
    ...
    const pdfColumns = ["S.No", "Date", "Reference", "Product", "Customer", "Warehouse", "Qty Sold", "Total", " Payment Status"]

    const getReport = async () => {
        try {
            setloading(true);
            const res = await DataService.get(`/get/product-sales/reports?startDate=${state[0].startDate}&endDate=${state[0].endDate}`);
                                ...
                                subHeaderComponent={
                                    <div className="d-flex gap-3 justify-content-end">
                                        ...
                                        <Button
                                            ...
                                            onclick={() => generatePDF('ProductSalesReport', pdfColumns, tableBody)}
                                        />
                                        <Button
                                            text='CSV'
                                            className='btn btn-success'
                                            onclick={() => downloadCSV('ProductSalesReport', data)}
                                        />
